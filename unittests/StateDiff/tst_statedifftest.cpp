...
void StateDiffTest::testCase1()
{
    ...
    QVERIFY2(stateDiff2.diffOps().size() == 1, "DiffOPs size incorret");
    QVariantList l = stateDiff2.diffOps().first().toList();
    qDebug() << "hello";
    QVERIFY2(l.takeFirst().toString() == sdo.action(), "action wrong");
